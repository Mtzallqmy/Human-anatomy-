...
export function SystemPage({ systemId }: { systemId: string }) {
  ...
  const profile = getSystemLearningProfile(systemId);
      ...
      <main className="editorial-main system-learning-main">
        <section className="editorial-hero system-learning-hero">
          <div>
            ...
            <p className="editorial-intro">{profile ? localize(profile.overview) : localize(system.description)}</p>
        ...
        <div className="system-metrics system-metrics-expanded">
          <div>
            <strong>{detailedStructures.length}</strong>
            <span>{t("systemPage.anatomyCount")}</span>
          </div>
          <div>
            <strong>{profile?.physiologyFocus.length ?? 0}</strong>
            <span>{label("Core physiology topics", "موضوعات فسيولوجية أساسية")}</span>
          </div>
          <div>
            <strong>{profile?.mechanisms.length ?? 0}</strong>
            <span>{label("Mechanisms explained", "آليات مشروحة")}</span>
          </div>
          <div>
        ...
        {profile && (
          <section className="system-learning-snapshot">
            <div className="editorial-section-heading">
              <div>
                <p className="eyebrow">{label("Chapter map", "خريطة الفصل")}</p>
                <h2>{label("What you will master", "ما الذي ستتقنه؟")}</h2>
              </div>
            </div>
            <div className="system-curriculum-grid">
              <article>
                <BookOpen size={20} />
                <h3>{label("Anatomy focus", "محاور التشريح")}</h3>
                <ul>
                  {profile.anatomyFocus.map((item) => <li key={item.en}>{localize(item)}</li>)}
                </ul>
              </article>
              <article>
                <Activity size={20} />
                <h3>{label("Physiology focus", "محاور الفسيولوجيا")}</h3>
                <ul>
                  {profile.physiologyFocus.map((item) => <li key={item.en}>{localize(item)}</li>)}
                </ul>
              </article>
              <article>
                <Gauge size={20} />
                <h3>{label("Control & regulation", "التحكم والتنظيم")}</h3>
                <ul>
                  {profile.regulation.map((item) => <li key={item.en}>{localize(item)}</li>)}
                </ul>
              </article>
            </div>
          </section>
        ...
        <section className="system-anatomy-section" id="anatomy">
          <div className="editorial-section-heading">
            <div>
              <p className="eyebrow">{label("Regional → structural", "من المنطقة إلى التركيب")}</p>
              <h2>{label("Anatomy, structure by structure", "التشريح، تركيبًا بعد تركيب")}</h2>
            </div>
            <span>{detailedStructures.length}</span>
          </div>
          <div className="anatomy-card-grid">
            {detailedStructures.map((structure, index) => (
              <Link key={structure.id} href={`/atlas/structure/${structure.id}`} className="anatomy-learning-card">
                <div className="anatomy-card-topline">
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <ArrowUpRight size={15} />
                </div>
                <h3>{localize(structure.name)}</h3>
                {structure.latinName && <small dir="ltr">{structure.latinName}</small>}
                <p>{localize(structure.anatomy)}</p>
                <div className="anatomy-location">
                  <MapPin size={13} />
                  <span>{localize(structure.location)}</span>
                </div>
              </Link>
            ))}
          </div>
        ...
        {profile && (
          <section className="physiology-deep-dive" id="physiology">
            <div className="editorial-section-heading">
              <div>
                <p className="eyebrow">{label("Function → mechanism → regulation", "الوظيفة ← الآلية ← التنظيم")}</p>
                <h2>{label("Physiology in depth", "الفسيولوجيا بتفصيل أعمق")}</h2>
              </div>
            </div>
              ...
              <div className="mechanism-grid">
                {profile.mechanisms.map((mechanism, index) => (
                  <article className="mechanism-card" key={mechanism.title.en}>
                    <span>{String(index + 1).padStart(2, "0")}</span>
                    <h3>{localize(mechanism.title)}</h3>
            ...
            <Link href={`/atlas/${system.slug}`} className="system-atlas-callout">
              <span>
                <Activity size={18} />
                <strong>{label("See the physiology inside the 3D atlas", "شاهد الفسيولوجيا داخل الأطلس ثلاثي الأبعاد")}</strong>
                <small>{label("Select structures, follow pathways and switch between anatomy, physiology, pathology and imaging.", "حدد التراكيب وتتبع المسارات وانتقل بين التشريح والفسيولوجيا والمرض والتصوير.")}</small>
              </span>
        ...
        {profile && (
          <section className="clinical-integration-section" id="clinical">
            <div className="editorial-section-heading">
              <div>
                <p className="eyebrow">{label("Clinical integration", "الربط السريري")}</p>
                <h2>{label("From normal function to disease", "من الوظيفة الطبيعية إلى المرض")}</h2>
              </div>
            </div>
            <div className="clinical-link-grid">
              {profile.clinicalLinks.map((item, index) => (
                <article key={item.en}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <p>{localize(item)}</p>
                </article>
        ...
        <section className="disease-module-section">
          ...
          {diseases.length ? (
            ...
            <div className="system-empty-clinical">
              <Stethoscope size={22} />
              <div>
                <strong>{label("Pathology module ready for expansion", "وحدة الأمراض جاهزة للتوسع")}</strong>
                <p>{label("This chapter currently prioritizes normal anatomy and physiology. Disease cases can be added through the existing review workflow without redesigning the page.", "يركز هذا الفصل حاليًا على التشريح والفسيولوجيا الطبيعيين، ويمكن إضافة الحالات المرضية عبر مسار المراجعة الحالي دون إعادة تصميم الصفحة.")}</p>
              </div>
            </div>
          )}
        </section>
