...
function initialTheme(): ThemeMode {
  ...
  return stored && options.some((item) => item.id === stored) ? stored : "comfort";
}
...
export function ThemeSwitcher() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("anatomica-theme", theme);
  }, [theme]);
  ...
  return (
    <div className="theme-switcher" role="group" aria-label="Display theme / نمط العرض">
      {options.map((option) => {
        ...
        return (
          <button key={option.id} type="button" className={theme === option.id ? "is-active" : ""} onClick={() => setTheme(option.id)} title={`${option.en} — ${option.ar}`} aria-pressed={theme === option.id}>
